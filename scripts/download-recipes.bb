...
(def recipe-folder "./src/content/beers")
...
(defn last-recipe!
  []
  (if-not (empty? (fs/list-dir recipe-folder))
    (-> (fs/list-dir recipe-folder)
        ;; Sort by file name
        sort
        last
        fs/read-all-bytes
        ...
        :_id
        (try (catch Exception _ (throw (ex-info "could not parse last recipe" {})))))
    nil))
...
(defn write-recipe!
  [recipe]
  (let [slug (str (-> recipe :_timestamp_ms timestamp->date-str)
                  "-"
    ...
    (spit (str recipe-folder
               ...
               slug
               ".json")
          (json/generate-string (assoc recipe :slug slug) {:pretty true}))))
...
(comment
  ...
  (list-remote-recipes! (basic-auth client (op-secret-memoized!))
                        nil)
  (as-> (op-secret!) $
    (basic-auth client $)
    (list-remote-recipes! $ (last-recipe!))
    ...
    (doseq [id $]
      (println "Fetcing recipe" id)
      (-> (fetch-remote-recipe! (basic-auth client (op-secret-memoized!)) id)
          (write-recipe!)))))